...
export type DomeImage = string | { src: string; alt?: string }
...
const getDataNumber = (el: HTMLElement, name: string, fallback: number) => {
  ...
  const attr = el.dataset[camel] ?? el.getAttribute(`data-${name}`)
  const n = attr == null ? NaN : parseFloat(attr)
  return Number.isFinite(n) ? n : fallback
}

function buildItems(pool: DomeImage[], seg: number): DomeItem[] {
  ...
  useGesture(
    {
      onDragStart: ({ event }) => {
        ...
        stopInertia()
        const evt = event as PointerEvent | MouseEvent
